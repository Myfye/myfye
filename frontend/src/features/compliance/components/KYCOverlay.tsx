...
import { toggleModal } from "../stores/kycSlice";
    ...
    const fetchKYCStatus = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            `Failed to fetch KYC status: ${response.status} ${response.statusText}`
          );
        }
    ...
    if (kycStatus === "APPROVED") {
      ...
      try {
        ...
        const response = await fetch(
          `${MYFYE_BACKEND}/generate_sumsub_external_link`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
    ...
    >
      {loading ? (
        ...
        >
          ...
          <section
            css={css`
              margin-block-start: ${kycStatus === "REJECTED"
                ? "calc(var(--size-200) + 48px + var(--size-200))"
                : "var(--size-100)"};
            ...
            >
              ...
              >
                ...
                >
                  To continue,&nbsp;
                  <span
                    style={{ color: "#006BCC", textDecoration: "underline" }}
                  >
                    <a
