...
import { selectAssetsWithBalanceByDashboard } from "../../assets/stores/assetsSlice";
import ensureTokenAccount from "../../../functions/ensureTokenAccount";
import { getMintAddress } from "../../assets/stores/assetsSlice";
import SelectAssetOverlay from "../../assets/components/SelectAssetOverlay";
...
const SelectSwapAssetOverlay = ({ zIndex = 1000 }) => {
  ...
  const onAssetSelect = (assetId: Asset["id"]) => {
    ...
    dispatch(
      updateExchangeRate({
        buyAssetId:
          transactionType === "buy" ? assetId : transaction.buy.assetId,
        sellAssetId:
          transactionType === "sell" ? assetId : transaction.sell.assetId,
        assets: assets,
      })
    );
    dispatch(
  ...
  const ensureTokenAccountForSwap = (assetId: Asset["id"]) => {
    console.log(
