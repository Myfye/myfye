...
import {
  selectAssetIdFromMintAddress,
  selectAssetTickerFromMintAddress,
} from "../../assets/stores/assetsSlice";
...
const ActivityList = ({ transactions }: Props) => {
  const assets = useAppSelector((state) => state.assets.assets);
  const getAssetIdFromMintAddress = useSelector(selectAssetIdFromMintAddress);
  const getAssetTickerFromMintAddress = useSelector(
    selectAssetTickerFromMintAddress
  );
  ...
  const calculateUSDValue = (amount: number, mintAddress: string) => {
    ...
    if (!assetIdFromMint || !assets[assetIdFromMint]) {
      console.log("Asset not found:", {
        assetIdFromMint,
        hasAsset: !!assets[assetIdFromMint],
      });
    ...
    console.log("Final calculation:", {
      amount,
      exchangeRateUSD: asset.exchangeRateUSD,
      result,
    });
  ...
  const getTransactionIcon = (type: string) => {
    ...
    switch (type) {
      case "swap":
        return <div css={baseIconStyle}>⇄</div>;
      case "deposit":
        return <div css={baseIconStyle}>↓</div>;
      case "withdraw":
        return <div css={baseIconStyle}>↑</div>;
    ...
    switch (type) {
      case "swap":
        if (transaction && transaction.input_mint && transaction.output_mint) {
          ...
          const inputMint = getAssetTickerFromMintAddress(
            transaction.input_mint
          );
          const outputMint = getAssetTickerFromMintAddress(
    ...
    switch (transaction.type) {
      case "swap":
        return `${formatAmount(
          ...
          transaction.type
        )} → ${formatAmount(transaction.output_amount, transaction.type)}`;
      case "deposit":
