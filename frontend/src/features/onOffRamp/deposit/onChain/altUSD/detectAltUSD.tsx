...
import {
  ...
  updateExchangeRate,
} from "@/features/swap/stores/swapSlice";
import { updateBalance } from "@/features/assets/stores/assetsSlice";
...
const AltUSDModal = () => {
  ...
  const { isOpen: modalIsOpen } = useSelector(
    (state: RootState) => state.altUSD.modal
  );
    ...
    if (transaction.status === "success") {
      ...
      if (amountToAdd > 0) {
        ...
        if (selectedOutputCurrency === "USD") {
          ...
          console.log(
            `DETECT ALT USD Adding ${amountToAdd} USD worth of USDY (${usdyAmountToAdd} tokens at $${usdyPrice} each) from USDY swap ${currentUSDYBalance} + ${usdyAmountToAdd} = ${
              currentUSDYBalance + usdyAmountToAdd
            }`
          );
          dispatch(
            updateBalance({
          ...
          console.log(
            `DETECT ALT USD Adding ${amountToAdd} USD worth of QQQ (${qqqAmountToAdd} shares at $${qqqPrice} each) from QQQ swap ${currentQQQBalance} + ${qqqAmountToAdd} = ${
              currentQQQBalance + qqqAmountToAdd
            }`
          );
          dispatch(
            updateBalance({
  ...
  useEffect(() => {
    if (usdtBalance > 0.01) {
      console.log(
        "DETECT ALT USD  USDT balance detected, should trigger modal:",
        usdtBalance
      );
    }
    if (pyusdBalance > 0.01) {
      console.log(
        "DETECT ALT USD  PYUSD balance detected, should trigger modal:",
        pyusdBalance
      );
    }
        ...
        >
          ...
          >
            <div
              ...
              `}
            >
              <CashIcon size={30} color="var(--clr-primary)" weight="light" />
            </div>
        ...
        >
          ...
          >
            <div
              ...
              `}
            >
              <SwapIcon size={30} color="var(--clr-primary)" weight="light" />
            </div>
        ...
        >
          ...
          >
            <div
              ...
              `}
            >
              <BankIcon size={30} color="var(--clr-primary)" weight="light" />
            </div>
        ...
        >
          ...
          >
            <div
              ...
              `}
            >
              <ChartIcon size={30} color="var(--clr-primary)" weight="light" />
            </div>
